...
func (sesid *sessionagent) SendByQueue(topic string, body []byte) string {
	if sesid.app == nil {
		return fmt.Sprintf("Module.App is nil")
	}
	server, e := sesid.app.GetServerByID(sesid.session.ServerId)
	...
	_, err := server.Call(nil, "SendByQueue", log.CreateTrace(sesid.TraceId(), sesid.SpanId()), sesid.session.SessionId, topic, body)
	return err
}
