...
func (this *MyTable) ControlDealerPoker() {
	...
	for k, v := range this.PlayerList {
		if v.IsHavePeople && v.Ready {
			...
			this.CalcPhomPoker(k, 0, this.PlayerList[k].HandPoker, [][]int{})
			pokerVal = this.CalcPhomData.maxV
			if straightData.Type > 0 && straightData.Idx == k {
				pokerVal = 3*0xFFFFFF + 1
			}
			ctlData := ControlData{
	...
	if len(realPlayerIdx)+len(robotPlayerIdx) != len(controlData) {
		log.Info("cardPhom control error")
		return
	}
