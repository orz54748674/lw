...
func (self *Room) OnInit(app module.App, settings *conf.ModuleSettings) {
	...
	self.GetServer().RegisterGO("/cardQzsg/onDisconnect", self.onDisconnect)
	common.AddListener(self.GetServerID(),common.EventDisconnect,"/cardQzsg/onDisconnect")
	...
	hook.RegisterAndCheckLogin(self.GetServer(),protocol.GrabDealer,self.GrabDealer)
	hook.RegisterAndCheckLogin(self.GetServer(),protocol.XiaZhu,self.XiaZhu)
