...
type Room struct {
	...
	RoomRobotConf       []cardSssStorage.RobotConf
...
func (self *Room) OnInit(app module.App, settings *conf.ModuleSettings) {
	...
	self.GetServer().RegisterGO("/cardSss/onDisconnect", self.onDisconnect)
	common.AddListener(self.GetServerID(), common.EventDisconnect, "/cardSss/onDisconnect")
	...
	hook.RegisterAndCheckLogin(self.GetServer(), protocol.ShowPoker, self.ShowPoker)
	hook.RegisterAndCheckLogin(self.GetServer(), protocol.CancelShowPoker, self.CancelShowPoker)
	...
	hook.RegisterAndCheckLogin(self.GetServer(), protocol.GetPokerType, self.GetPokerType)
