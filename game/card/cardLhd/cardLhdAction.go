...
func (this *MyTable) DealXiaZhu(session gate.Session,xiaZhuV int64,xiaZhuPos cardLhdStorage.XiaZhuResult,idx int,userID string){
	...
	activityStorage.UpsertGameDataInBet(session.GetUserID(),game.CardLhd,1)
}
func (this *MyTable) XiaZhu(session gate.Session, msg map[string]interface{}) (err error) {
	...
	xiaZhuTotal := xiaZhuV
	for _,v := range this.PlayerList[idx].XiaZhuResultTotal{
			xiaZhuTotal += v
	}

	if xiaZhuTotal > BetLimit{
	...
	bill := walletStorage.NewBill(userID,walletStorage.TypeExpenses,walletStorage.EventGameCardLhd,this.EventID,-xiaZhuV)
...
func (this *MyTable) DoubleXiaZhu(session gate.Session, msg map[string]interface{}) (err error) {
	...
	var XiaZhuResult map[cardLhdStorage.XiaZhuResult][]int64
	XiaZhuResult = map[cardLhdStorage.XiaZhuResult][]int64{}
	...
	bill := walletStorage.NewBill(userID,walletStorage.TypeExpenses,walletStorage.EventGameCardLhd,this.EventID,-needYxb)
	walletStorage.OperateVndBalance(bill)
	this.notifyWallet(userID)
...
func (this *MyTable) GetShortCutList(session gate.Session, msg map[string]interface{}) (err error) {
	...
	if this.ShortCut[userID] == nil{
		this.ShortCut[userID] = game.ShortCut[game.CardLhd]
	}
