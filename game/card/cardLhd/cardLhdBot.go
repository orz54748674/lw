...
func (this *MyTable) GetXiaZhuResultMaxMin(src map[cardLhdStorage.XiaZhuResult]int64) (maxPos cardLhdStorage.XiaZhuResult, minPos cardLhdStorage.XiaZhuResult) {
	type Dest struct {
		Key cardLhdStorage.XiaZhuResult
		V   int64
	}
	dest := []Dest{}
	for k, v := range src {
	...
	for k, v := range this.PlayerList {
		if v.Role == ROBOT {
			if k < this.PositionNum {
				...
				xiaZhuTotal := this.RandInt64(15, 30) * v.Yxb / 1000              //获取总共下注筹码
				xiaZhuSplice := xiaZhuTotal * 2 / int64(this.GameConf.XiaZhuTime) //下注筹码分片
				for i := len(this.GameConf.PlayerChipsList); i > 0; i-- {         //获取筹码
					if xiaZhuSplice >= int64(this.GameConf.PlayerChipsList[i-1]) {
						xiaZhuSplice = int64(this.GameConf.PlayerChipsList[i-1])
...
func (this *MyTable) CalcChangeRobot(curBotNum int, robotConf cardLhdStorage.RobotConf, start bool) map[int]int { //计算变换机器人
	...
	for i := 0; i < robotConf.StepNum; i++ {
		...
		if rand >= 1 && rand < 2 {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_30_50_M))
			needRobotType[robotType] += addV
		} else if rand >= 2 && rand < 3 {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_10_30_M))
			needRobotType[robotType] += addV
		} else if rand >= 3 && rand < 22 {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_1_10_M))
			needRobotType[robotType] += addV
		} else if rand >= 22 && rand < 35 {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_500_1_M))
			needRobotType[robotType] += addV
		} else if rand >= 35 && rand < 55 {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_100_500_K))
			needRobotType[robotType] += addV
		} else if rand >= 55 && rand < 58 {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_50_100_K))
			needRobotType[robotType] += addV
		} else if rand >= 58 && rand < 84 {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_20_50_K))
			needRobotType[robotType] += addV
		} else if rand >= 84 && rand < 92 {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_1_20_K))
			needRobotType[robotType] += addV
		} else {
			robotType, _ := strconv.Atoi(string(cardLhdStorage.Robot_0_1_K))
			needRobotType[robotType] += addV
		}
	}
...
func (this *MyTable) RobotAdd(start bool) { //添加机器人
	...
	robotConf := cardLhdStorage.GetTableRobotConfByHour(this.tableID, startHour)
...
func (this *MyTable) RobotEnter(robot common2.Bot, tp int) bool {
	...
	pl := PlayerList{
		...
		XiaZhuResult:     map[cardLhdStorage.XiaZhuResult][]int64{},
		LastXiaZhuResult: map[cardLhdStorage.XiaZhuResult][]int64{},
...
func (this *MyTable) RobotDealXiaZhu(xiaZhuV int64, xiaZhuPos cardLhdStorage.XiaZhuResult, robot PlayerList) {
	...
	info := struct {
		UserID    string
		XiaZhuPos cardLhdStorage.XiaZhuResult
		XiaZhuV   int64
	}{
		UserID:    robot.UserID,
		XiaZhuPos: xiaZhuPos,
...
func (this *MyTable) RobotXiaZhu(robot PlayerList, msg map[string]interface{}) (err error) {
	...
	xiaZhuPos := cardLhdStorage.XiaZhuResult(msg["pos"].(string)) //鱼虾蟹下的哪一种图案
	if xiaZhuPos != cardLhdStorage.LONG &&
		xiaZhuPos != cardLhdStorage.HU &&
		xiaZhuPos != cardLhdStorage.HE {
