...
func (this *MyTable) GetTableInfo(userID string) interface{} {
	...
	tableInfo["RoundNum"] = this.RoundNum
...
func (this *MyTable) DealPoker(pool []int) {
	...
	//this.FirstPutIdx = 0
	//this.PlayerList[0].HandPoker = []int{69,70,57,27,61,78}
	//this.PlayerList[1].HandPoker = []int{35,52,22,25,58,62}
	//this.PlayerList[2].HandPoker = []int{29,34,55,57,58,61}
	//this.PlayerList[3].HandPoker = []int{51,56,59,60,71,78}
	//this.PlayerList[4].HandPoker = []int{24,27,43,50,66,72}
	//this.PlayerList[5].HandPoker = []int{23,35,42,56,68,76}
	...
	for k, v := range this.PlayerList {
		if v.Ready {
			sort.Slice(this.PlayerList[k].HandPoker, func(i, j int) bool { //升序排序
				if this.PlayerList[k].HandPoker[i]%0x10 == this.PlayerList[k].HandPoker[j] {
...
func (this *MyTable) InitCurRoundList() {
	...
	for _, v1 := range this.PutOverRecord {
		this.CurRoundList[v1.Idx] = false
	}
}
func (this *MyTable) PlayerIsOver(idx int) bool {
