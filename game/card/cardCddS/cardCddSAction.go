...
func (this *MyTable) PutPoker(userID string,msg map[string]interface{})  (err error) {
	...
	if (!waitingList.FirstRound && !this.CompBigger(this.LastPutCard,poker)) || !this.CurRoundList[idx]{
		if sb != nil {
			this.sendPack(s.GetSessionID(), game.Push, "", protocol.PutPoker, errCode.InvalidPutPoker)
		}
		return nil
	...
	pressScore := int64(0)
	if !waitingList.FirstRound && !this.IsPutOver(this.LastPutIdx){ //统计被压牌型
		pressScore = this.GetPokerPressList(pkType)
		if pressScore > 0{
			for _,v := range this.CurRoundPressList.Single2{
				if v / 0x10 == 4 || v / 0x10 == 3{
					this.CurRoundPressList.Score += this.BaseScore
				}else{
					this.CurRoundPressList.Score += this.BaseScore / 2
				}
			}
			this.CurRoundPressList.Single2 = make([]int,0)
			this.CurRoundPressList.PressIdx = this.LastPutIdx
	...
	if len(this.PlayerList[idx].HandPoker) == 0{
		...
		for k,v := range this.PlayerList{
			if v.Ready{
				if len(v.HandPoker) != 13 && k != idx{
					isSpring = false
					break
				}
			}
		}
		this.PutOverRecord = append(this.PutOverRecord,PutOverRecord{Idx: idx,Over: true,LastPutCard: poker,IsSpring: isSpring})
		data["putRank"] = len(this.PutOverRecord) - 1
		data["isSpring"] = isSpring
		this.RemoveCurRoundList(idx)

		if len(this.PutOverRecord) == 1{
			for k,v := range this.PlayerList{
				if v.Ready{
					if len(v.HandPoker) == 13{
						this.SpringList[k] = true
						this.RemoveCurRoundList(k)
					}
				}
			}
		}
	...
	if this.CheckGameOver(){
		undoIdx := -1
		if !this.PutOverRecord[0].IsSpring{
			for k,v := range this.PlayerList{
				if v.IsHavePeople && v.Ready && !this.SpringList[k]{
					find := false
					...
					if !find{
						undoIdx = k
						this.PutOverRecord = append(this.PutOverRecord,PutOverRecord{Idx: undoIdx,Over: false})
						break
					}
				}
			}
			data["putRank"] = len(this.PutOverRecord) - 2
		}else{
			data["putRank"] = 0
		}

		nextIdx := undoIdx//this.GetNextPutIdx(idx)
		data["idx"] = idx
	...
	if this.GetCurRoundPlayerNum() == 0{
		...
		nextIdx = this.GetNextPutIdx(this.LastPutIdx)
	...
	if nextIdx == idx{
		...
		bigPk = this.CheckBiggerPoker(this.LastPutCard,this.PlayerList[nextIdx].HandPoker)
		if len(bigPk) > 0{
			canPut = true
		}
	}
	...
	if this.PlayerList[nextIdx].Role == ROBOT{
		this.CountDown = 0
	}else{
...
func (this *MyTable) HintPoker(userID string)  (err error) {
	...
	bigPk := this.CheckBiggerPoker(this.LastPutCard,this.PlayerList[idx].HandPoker)
	res := make(map[string]interface{},2)
	res["Poker"] = bigPk
	if sb != nil{
