...
func (self *Room) OnTimer60(){
	self.RoomRobotConf = cardCddSStorage.GetRoomRobotConf()
}
...
func (self *Room) RoomInit(){
	...
	gameConf := cardCddSStorage.GetRoomConf()
	if gameConf == nil{
		gameConf = &cardCddSStorage.Conf{
			ReadyTime: 5,
			PutPokerTime: 15,
			JieSuanTime: 5,
		...
		cardCddSStorage.InsertRoomConf(gameConf)
	}

	roomData := cardCddSStorage.RoomData{
		TablesInfo: map[string]cardCddSStorage.TableInfo{
		}, //
	}
	cardCddSStorage.UpsertTablesInfo(roomData.TablesInfo)
	self.InitHallInfo()
	gameRobotConf := cardCddSStorage.GetRoomRobotConf()
	for _,v := range BaseScoreList{
...
func (self *Room) Enter(session gate.Session, msg map[string]interface{}) (map[string]interface{},error) {
	...
	isNotAllow := lobbyStorage.QueryLobbyGameLayoutByName(game.CardCddS).IsNotAllowPlay
...
func (self *Room) Info(session gate.Session, msg map[string]interface{}) (map[string]interface{},error){
	...
	res["cardCddSServerId"] = self.BaseModule.GetServerID()
