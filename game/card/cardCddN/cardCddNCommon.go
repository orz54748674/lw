...
func (this *MyTable) GetTableInfo(userID string) interface{} {
	...
	tableInfo["LastPutCard"] = this.LastData.LastPutCard
...
func (this *MyTable) DealPoker(pool []int) {
	...
	if this.IsBlack3First || this.LastData.LastFirstIdx < 0 || !this.PlayerList[this.LastData.LastFirstIdx].Ready {
		firstPkList := []int{
		...
		if this.GetRealReadyPlayerNum() > 0 {
			log.Info("last put idx = %d", this.LastData.LastFirstIdx)
		}
		this.FirstPutIdx = this.LastData.LastFirstIdx
	}

	if this.GetRealReadyPlayerNum() > 0 { //机器人控制
		this.ControlDealerPoker()
	}
	//this.FirstPutIdx = 0
	//this.PlayerList[0].HandPoker = []int{0x1e,0x2e,0x3e}
	//this.PlayerList[1].HandPoker = []int{0x13,0x17,0x3f}
...
func (this *MyTable) GetPokerPressList(pkType PokerType) int64 {
	lastPkType, lastMax, _, _ := this.GetCardType(this.LastData.LastPutCard)
	score := int64(0)
	if lastPkType == Single && lastMax%0x10 == 0x0f { //单2被压
		...
		for _, v := range this.LastData.LastPutCard {
...
func (this *MyTable) AutoPutPoker(userId string, pk []int) {
	res := make(map[string]interface{})
	poker := make([]interface{}, 0)
	for _, v := range pk {
		poker = append(poker, v)
	}
	res["Poker"] = poker
	this.PutQueue(protocol.PutPoker, userId, res)
}
func (this *MyTable) InitBestCardData() {
	this.BestHandCard = BestHandCard{
		SumValue:   -100, //为了后面作对比
		Poker:      []int{},
		FirstPoker: []int{},
		NeedRound:  0,
	}
