...
func (self *Room) OnTimer60(){
	self.RoomRobotConf = cardCddNStorage.GetRoomRobotConf()
}
	...
	for _,v := range BaseScoreList{
		for _,v1 := range self.RoomRobotConf{
			if v1.HallType == string(FOUR) && v1.BaseScore == v{
				offset := int(self.RandInt64(1, int64(v1.MaxOffset) / 2)) / 2 * 2
...
func (self *Room) RoomInit(){
	gameStorage.UpsertGameReboot(game.CardCddN,"false")

	gameConf := cardCddNStorage.GetRoomConf()
	if gameConf == nil{
		...
		cardCddNStorage.InsertRoomConf(gameConf)
	}

	roomData := cardCddNStorage.RoomData{
		TablesInfo: map[string]cardCddNStorage.TableInfo{
		}, //
	}
	cardCddNStorage.UpsertTablesInfo(roomData.TablesInfo)
	self.InitHallInfo()
	gameRobotConf := cardCddNStorage.GetRoomRobotConf()
	for _,v := range BaseScoreList{
			if gameRobotConf == nil{
				self.RoomRobotInit(v)
			}
			self.CreateTable(strconv.FormatInt(v, 10) + "_" + strconv.FormatInt(1, 10) + "_4" )
			time.Sleep(time.Millisecond * 100)
			self.CreateTable(strconv.FormatInt(v, 10) + "_" + strconv.FormatInt(1, 10) + "_2" )
			time.Sleep(time.Millisecond * 100)
	}
	//self.CreateTable(strconv.FormatInt(100, 10) + "_" + strconv.FormatInt(1, 10) + "_2" )
...
func (self *Room) Enter(session gate.Session, msg map[string]interface{}) (map[string]interface{},error) {
	...
	isNotAllow := lobbyStorage.QueryLobbyGameLayoutByName(game.CardCddN).IsNotAllowPlay
