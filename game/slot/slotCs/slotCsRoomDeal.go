...
func (self *Room) OnTimer(){
	for k,v := range CoinNum {
		...
		rand = rand * v / 50
		slotCsStorage.IncJackpot(k,rand)
	}
}
func (self *Room) OnTimer10(){
	...
	if rand == 5{
		...
		if len(robot) > 0{
			slotStorage.InsertJackpotRecord(robot[0].Oid.Hex(),robot[0].NickName,coinVal,getJackpot,game.SlotCs,"","",nil)
			curJackpot := slotCsStorage.GetJackpot()
			for k1,_ := range CoinNum{
				val := -curJackpot[k1] + InitJackpot[k1]
				slotCsStorage.IncJackpot(k1,val)
			}
		}
	}
}
...
func (self *Room) Enter(session gate.Session, msg map[string]interface{}) (map[string]interface{},error) {
	...
	reboot := gameStorage.QueryGameReboot(game.SlotCs)
	if "true" == reboot{ //准备重启
...
func (self *Room) GetResults(session gate.Session, msg map[string]interface{}) (map[string]interface{},error) {
	//	table_id := msg["table_id"].(string)
	userID := session.GetUserID()
	var tableID string
	var table room.BaseTable
	...
	table.PutQueue(protocol.GetResults,session,msg)
	return  errCode.Success(nil).SetAction(game.Nothing).GetMap(),nil
}
...
func (self *Room) GetJackpot(session gate.Session,msg map[string]interface{}) (map[string]interface{},error) {
	...
	goldJackpot := slotCsStorage.GetJackpot()
	find := false
	for k,v := range CoinNum {
		if v == coinNum{
...
func (self *Room) Info(session gate.Session, msg map[string]interface{}) (map[string]interface{},error){
	res := make(map[string]interface{},2)
	res["slotCsServerId"] = self.BaseModule.GetServerID()
	goldJackpot := slotCsStorage.GetJackpot()
...
func (self *Room) EnterBonusGame(session gate.Session, msg map[string]interface{}) (map[string]interface{},error){
	...
	myTable.GameConf = slotCsStorage.GetRoomConf()
...
func (self *Room) EnterMiniGame(session gate.Session, msg map[string]interface{}) (map[string]interface{},error){
	...
	myTable := table.(*MyTable)
	if myTable.ModeType != NORMAL || myTable.JieSuanData.TotalBackScore <= 0 || myTable.JieSuanData.BonusGame{
		return errCode.ServerError.GetI18nMap(), nil
	}
	...
	myTable.GameConf = slotCsStorage.GetRoomConf()
	res["XiaZhu"] = myTable.JieSuanData.TotalBackScore
	res["Get"] =  myTable.JieSuanData.TotalBackScore * (2000 - int64(myTable.GameConf.ProfitPerThousand) * 2) / 1000
	myTable.CountDown = myTable.GameConf.BonusTimeOut
	myTable.BonusGameData.State = 1
	return errCode.Success(res).GetMap(), nil
}
...
func (self *Room) SelectBonusTimes(session gate.Session, msg map[string]interface{}) (map[string]interface{},error) {
	if msg["Serial"] == nil{
		return errCode.ServerError.GetI18nMap(), nil
	}
	userID := session.GetUserID()
	...
	myTable := table.(*MyTable)
	if !myTable.JieSuanData.BonusGame || myTable.BonusGameData.State != 2{
		return errCode.ServerError.GetI18nMap(), nil
	}
	table.PutQueue(protocol.SelectBonusTimes,session,msg)
	return  errCode.Success(nil).SetAction(game.Nothing).GetMap(),nil
}
...
func (self *Room) GetJackpotRecord(session gate.Session,params map[string]interface{}) (map[string]interface{},error) {
	...
	res := make(map[string]interface{},2)
	record:= slotStorage.QueryJackpotRecord(int(Offset),int(PageSize),game.SlotCs)
	res["JackpotRecord"] = record
	res["TotalNum"] = slotStorage.QueryJackpotRecordTotal(game.SlotCs)
	return  errCode.Success(res).GetMap(),nil
}