...
type MyTable struct {
	...
	ResultsChipList map[yxxStorage.XiaZhuResult][]int64 `bson:"XiaZhuResult" json:"XiaZhuResult"` //桌上下注筹码结果
	Results         map[string]yxxStorage.XiaZhuResult  `bson:"Results" json:"Results"`           //开奖结果图案

	XiaZhuTotal     map[yxxStorage.XiaZhuResult]int64 `bson:"XiaZhuTotal" json:"XiaZhuTotal"`         //桌上下注结果总数
	RealXiaZhuTotal map[yxxStorage.XiaZhuResult]int64 `bson:"RealXiaZhuTotal" json:"RealXiaZhuTotal"` //每轮真实玩家下注总和

	PositionNum  int
	PositionList []PlayerList `bson:"PositionList" json:"PositionList"`
	Rand         *rand.Rand
	PrizePool    int64 //奖池
	PrizeSwitch  bool  //开大奖状态
	PlayerNum    int   //人数
	CurInPool    int64
	EventID      string
	RobotYxbConf []yxxStorage.Robot
