...
var(
	cRoomData = "YxxRoomData"
	cRoomRecord = "YxxRoomRecord"
	cRoomConf = "YxxRoomConf"
	cRobotConf = "YxxRobotConf"
)
...
func GetPrizeRecord(tableID string) PrizeRecord {
	c := common.GetMongoDB().C(cRoomRecord)
	var roomRecord RoomRecord
	if err := c.Find(nil).One(&roomRecord); err != nil{
		log.Info("not found prize record ",err)
		return PrizeRecord{}
	}
	return roomRecord.PrizeRecord[tableID]
}
func UpsertPrizeRecord(prizeRecord PrizeRecord,tableID string) *common.Err {
	c := common.GetMongoDB().C(cRoomRecord)
	selector := bson.M{"nil":nil}
	update := bson.M{"$set":bson.M{"PrizeRecord":bson.M{tableID:prizeRecord}}}

	_,err :=c.Upsert(selector,update)
	if err != nil{
		log.Error("Upsert prize record error: %s", err)
		return nil //errCode.ServerError.SetErr(err.Error())
	}
	return nil
}
