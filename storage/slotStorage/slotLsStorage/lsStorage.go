...
var(
	cRoomData = "SlotLsRoomData"
	cRoomConf = "SlotLsRoomConf"
)
...
func IncJackpot(pos int,goldJackpot int64,silverJackpot int64){
	c := common.GetMongoDB().C(cRoomData)
	update := bson.M{"$inc":bson.M{
		"GoldJackpot."+strconv.Itoa(pos):goldJackpot,"SilverJackpot."+strconv.Itoa(pos):silverJackpot,
	}}
	if _,err := c.Upsert(nil, update);err !=nil{
		log.Error(err.Error())
	}
}
func GetJackpot() (goldJackpot []int64,silverJackpot []int64){
	c := common.GetMongoDB().C(cRoomData)
	var roomData RoomData
	if err := c.Find(nil).One(&roomData); err != nil{
	...
	return roomData.GoldJackpot,roomData.SilverJackpot
}
